...
import { formUtils } from "../utils/index.ts";
const { initializeRules } = formUtils();
...
if (column && column.rules && Array.isArray(column.rules)) {
  rules.value = [...rules.value, ...initializeRules(column.rules, props)];
}
  ...
  get: function () {
    return column.precision
      ? typeof props.data == "number" &&
          parseFloat(props.data?.toFixed(column.precision))
      : props.data;
  },
  set: function (val) {
